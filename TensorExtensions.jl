...
and negative integers for free indices. Optional input 'order'  can be used
...
function ncon!(
  ...
  if !(connect_list[1] isa Array || connect_list[1] isa UnitRange)
  ...
  if isempty(order)
    order = sort(unique(flat_connect[flat_connect.>0]))
  end
  ...
  if check_network
    ...
    ncon_check_inputs(connect_list, flat_connect, dims_list, order)
  end
  ...
  for ip = 1:length(connect_list)
    ...
    if num_cont > 0
      @assert op==* # I didn't implement the tropical_matrix_mult for this
      tensor_list[ip], connect_list[ip], cont_label =
        ncon_partial_trace(tensor_list[ip], connect_list[ip])
      order = setdiff(order, cont_label)
    end
  end
...
"""
function ncon_tensordot(A, B, A_cont, B_cont; op = *)
...
function ncon_partial_trace(A, A_label)
  ...
  if num_cont > 0
    ...
    B = sum([A[:,ip,ip] for ip=1:cont_dim])
...
"""
function ncon_check_inputs(connect_list, flat_connect, dims_list, order)
  ...
  if !(sort(order) == sort(unique(pos_ind)))
